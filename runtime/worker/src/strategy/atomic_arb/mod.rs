...
impl AtomicArbStrategy {
    ...
    async fn find_arbitrage_routes(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match lifi::find_routes(search_params).await {
            Ok(response) => {
                ...
                match build_best_route_transaction(&response).await {
                    Ok(tx) => {
...
impl crate::strategy::Strategy for AtomicArbStrategy {
    fn execute(&self) {
        ...
        runtime.block_on(async {
            if let Err(e) = self.find_arbitrage_routes().await {
